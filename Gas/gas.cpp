...
void Flatulan::move()
{
    ...
    int player_r = m_city->player()->row();
    int player_c = m_city->player()->col();
    
    switch (dir)
    {
        case UP:
            if(m_row-1 != player_r || m_col != player_c) {
        ...
        case DOWN:
            ...
            break;
        case LEFT:
            if(m_col-1 != player_c || m_row != player_r) {
                if(!(m_city->determineNewPosition(m_row, m_col, dir)))
                    return;
            }
            break;
...
void City::display() const
{
    ...
    for (int r = 0; r < rows(); r++){
        for (int c = 0; c < cols(); c++){
            numOfF = nFlatulansAt(r+1, c+1);
            total += numOfF;
            if (numOfF == 1)
                grid[r][c] = 'F';
            else if (numOfF == 1 || numOfF == 2 || numOfF == 3 || numOfF == 4 ||
                     numOfF == 5 || numOfF == 6 || numOfF == 7 || numOfF == 8)
                grid[r][c] = numOfF + '0';
        }
    }
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    if(m_player->row() == 1) { i++; }
    if(m_player->row() == rows()) { m--; }
    if(m_player->col() == 1) { j++; }
    if(m_player->col() == cols()) { n--; }
    
    for(; i < m; i++){
        for(int w = j; w < n; w++){
            if(nFlatulansAt(m_player->row()+i, m_player->col()+w) > 0){
                for(int k = 0; k < m_nFlatulans; k++){
                    if(m_flatulans[k] != nullptr){
                        if(m_flatulans[k]->row()==m_player->row()+i &&
                           m_flatulans[k]->col()==m_player->col()+w &&
                           m_flatulans[k]->possiblyGetConverted()){
                            delete m_flatulans[k];
                            for(int x = k + 1; x < m_nFlatulans; x++) {
                                m_flatulans[x - 1] = m_flatulans[x];
                            }
                            m_nFlatulans--;
                        }
                    }
...
void City::moveFlatulans()
{
    ...
    for (int k = 0; k < m_nFlatulans; k++){
        ...
        for(; i < m; i++){
            for(int w = j; w < n; w++){
                if(((i==-1 || i==1) && w==0) || (i==0)){
                    if(m_flatulans[k]->row() == m_player->row()+i && m_flatulans[k]->col() == m_player->col()+w)
                        m_player->getGassed();
                }
            }
        }
